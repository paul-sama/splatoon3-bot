...
@on_command("show_db_info", aliases={'sdi'}, priority=10, block=True).handle(parameterless=[Depends(_check_session_handler)])
...
@on_command("last", priority=10, block=True, ).handle(parameterless=[Depends(_check_session_handler)])
...
@on_command("ns_friends", aliases={'ns'}, priority=10, block=True).handle(parameterless=[Depends(_check_session_handler)])
...
@on_command("my_schedule", priority=10, block=True).handle(parameterless=[Depends(_check_session_handler)])
async def schedule(bot: Bot, event: Event):
    msg = await get_my_schedule_msg(event.get_user_id())
    await bot_send(bot, event, msg, parse_mode='Markdown')


@on_command("screen_shot", aliases={'ss'}, priority=10, block=True).handle(parameterless=[Depends(_check_session_handler)])
...
@on_command("history", priority=10, block=True).handle(parameterless=[Depends(_check_session_handler)])
...
@on_command("friend_code", aliases={'fc'}, priority=10, block=True).handle(parameterless=[Depends(_check_session_handler)])
async def friend_code(bot: Bot, event: Event):
    msg = get_friend_code(event.get_user_id())
    await bot_send(bot, event, msg, parse_mode='Markdown')


@on_command("top", priority=10, block=True).handle(parameterless=[Depends(_check_session_handler)])
