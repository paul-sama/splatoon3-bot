...
async def bot_send(bot: Bot, event: Event, message: str, **kwargs):
    ...
    if img_data:
        ...
        if isinstance(bot, V11_Bot):
            await bot.send(event, message=V11_MsgSeg.image(file=img_data, cache=False), reply_message=True)
    ...
    try:
        if isinstance(bot, V11_Bot):
            r = await bot.send(event, message=V11_MsgSeg.text(message), reply_message=True)
        elif isinstance(bot, Kook_Bot):
